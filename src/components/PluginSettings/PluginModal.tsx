...
const Components: Record<
    OptionType,
    React.ComponentType<ISettingElementProps<any>>
> = {
...
export default function PluginModal({
        ...
        (async () => {
            for (const user of plugin.authors.slice(0, 6)) {
                const author = user.id
                    ? await UserUtils.getUser(`${user.id}`).catch(() =>
    ...
    async function saveAndClose() {
        ...
        if (plugin.beforeSave) {
            const result = await Promise.resolve(
                plugin.beforeSave(tempSettings),
            );
            if (result !== true) {
                setSaveError(result);
                return;
    ...
    function renderSettings() {
        if (!hasSettings || !plugin.options) {
            ...
            const options = Object.entries(plugin.options).map(
                ([key, setting]) => {
