...
export default definePlugin({
    ...
    stringDelta(delta: number, showMillis: boolean) {
        ...
        const str = (k: DiffKey) =>
            diff[k] > 0
                ? `${diff[k]} ${diff[k] > 1 ? k : k.substring(0, k.length - 1)}`
                : null;
    ...
    latencyTooltipData(message: Message) {
        const { latency, detectDiscordKotlin, showMillis } =
            this.settings.store;
        ...
        let delta =
            SnowflakeUtils.extractTimestamp(id) -
            SnowflakeUtils.extractTimestamp(nonce); // milliseconds
        if (!showMillis) {
        ...
        const stringDelta =
            abs >= latencyMillis ? this.stringDelta(abs, showMillis) : null;
