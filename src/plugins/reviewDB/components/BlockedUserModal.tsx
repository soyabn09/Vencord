...
function BlockedUser({
    ...
    return (
        <div className={cl("block-modal-row")}>
            ...
            <Forms.FormText className={cl("block-modal-username")}>
                {user.username}
            </Forms.FormText>
            <UnblockButton
                onClick={
...
function Modal() {
    ...
    const [blocks, error, pending] = useAwaiter(fetchBlocks, {
        onError: (e) =>
            new Logger("ReviewDB").error("Failed to fetch blocks", e),
        fallbackValue: [],
    });
    ...
    if (error)
        return (
            <Forms.FormText>
                Failed to fetch blocks: ${String(error)}
            </Forms.FormText>
        );
    if (!blocks.length)
        ...
        <ModalRoot {...modalProps}>
            ...
            <ModalContent className={cl("block-modal")}>
                {Auth.token ? (
                    ...
                    <Forms.FormText>
                        You are not logged into ReviewDB!
                    </Forms.FormText>
                )}
            </ModalContent>
        </ModalRoot>
