...
const { updateGuildNotificationSettings } = findByPropsLazy(
    "updateGuildNotificationSettings",
);
const { toggleShowAllChannels } = mapMangledModuleLazy(
    ".onboardExistingMember(",
    {
        toggleShowAllChannels: (m) => {
            ...
            return (
                s.length < 100 &&
                !s.includes("onboardExistingMember") &&
                !s.includes("getOptedInChannels")
            );
        },
    },
);
const isOptInEnabledForGuild = findByCodeLazy(
    ".COMMUNITY)||",
    ".isOptInEnabled(",
);
    ...
    patches: [
        {
            ...
            replacement: {
                ...
                replace: (m, guildId) =>
                    `${m}$self.applyDefaultSettings(${guildId});`,
            },
        },
        {
            find: "{joinGuild:",
            replacement: {
                ...
                replace: (m, guildId, lurker) =>
                    `${m}if(!${lurker})$self.applyDefaultSettings(${guildId});`,
            },
        },
    ],
    settings,
