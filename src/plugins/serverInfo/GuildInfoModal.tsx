...
function GuildInfoModal({ guild }: GuildProps) {
    ...
    const bannerUrl =
        guild.banner &&
        IconUtils.getGuildBannerURL(guild, true)!.replace(
            /\?size=\d+$/,
            "?size=1024",
        );
        ...
        <div className={cl("root")}>
            ...
            <div className={cl("header")}>
                ...
                <div className={cl("name-and-description")}>
                    <Forms.FormTitle tag="h5" className={cl("name")}>
                        {guild.name}
                    </Forms.FormTitle>
                    {guild.description && (
                        <Forms.FormText>{guild.description}</Forms.FormText>
                    )}
                </div>
            </div>
            ...
            >
                <TabBar.Item
                    className={cl("tab", {
                        selected: currentTab === Tabs.ServerInfo,
                    })}
                    id={Tabs.ServerInfo}
                >
                    Server Info
                ...
                <TabBar.Item
                    className={cl("tab", {
                        selected: currentTab === Tabs.Friends,
                    })}
                    id={Tabs.Friends}
                >
                    Friends
                ...
                <TabBar.Item
                    className={cl("tab", {
                        selected: currentTab === Tabs.BlockedUsers,
                    })}
            ...
            <div className={cl("tab-content")}>
                {currentTab === Tabs.ServerInfo && (
                    <ServerInfoTab guild={guild} />
                )}
                {currentTab === Tabs.Friends && (
                    <FriendsTab guild={guild} setCount={setFriendCount} />
                )}
                {currentTab === Tabs.BlockedUsers && (
                    <BlockedUsersTab guild={guild} setCount={setBlockedCount} />
                )}
            </div>
        </div>
    );
...
function ServerInfoTab({ guild }: GuildProps) {
    ...
    const Fields = {
        ...
        "Created At": renderTimestamp(
            SnowflakeUtils.extractTimestamp(guild.id),
        ),
        "Joined At": guild.joinedAt
            ? renderTimestamp(guild.joinedAt.getTime())
            : "-", // Not available in lurked guild
        "Vanity Link": guild.vanityURLCode ? (
            <a>{`discord.gg/${guild.vanityURLCode}`}</a>
        ) : (
            "-"
        ), // Making the anchor href valid would cause Discord to reload
...
function BlockedUsersTab({ guild, setCount }: RelationshipProps) {
    const blockedIds = Object.keys(RelationshipStore.getRelationships()).filter(
        (id) => RelationshipStore.isBlocked(id),
    );
...
function UserList(
    ...
    for (const id of ids) {
        if (GuildMemberStore.isMember(guild.id, id)) members.push(id);
