...
export default definePlugin({
    ...
    renderUsername: ErrorBoundary.wrap(
        (props: { user: User; username: string }) => {
    ...
    renderRoleIcon: ErrorBoundary.wrap(
        ({ roleId, guildId }: { roleId: string; guildId: string }) => {
...
function getUsernameString(username: string) {
    return settings.store.showAtSymbol ? `@${username}` : username;
}
