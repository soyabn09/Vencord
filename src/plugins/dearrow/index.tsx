...
async function embedDidMount(this: Component<Props>) {
    try {
        ...
        if (
            ...
            const replacementTitle = titles[0].title.replace(
                /(^|\s)>(\S)/g,
                "$1$2",
            );

            embed.dearrow.oldTitle = dearrowByDefault
                ? embed.rawTitle
                : replacementTitle;
        ...
        if (hasThumb && replaceElements !== ReplaceElements.ReplaceTitlesOnly) {
            ...
            embed.dearrow.oldThumb = dearrowByDefault
                ? embed.thumbnail.proxyURL
                : replacementProxyURL;
            if (dearrowByDefault)
                embed.thumbnail.proxyURL = replacementProxyURL;
        }
        ...
        options: [
            {
                ...
                value: ReplaceElements.ReplaceAllElements,
                default: true,
            },
            { label: "Titles", value: ReplaceElements.ReplaceTitlesOnly },
            {
                label: "Thumbnails",
                value: ReplaceElements.ReplaceThumbnailsOnly,
            },
        ],
    },
    dearrowByDefault: {
