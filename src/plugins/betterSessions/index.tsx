...
const TimestampClasses = findByPropsLazy(
    "timestampTooltip",
    "blockquoteContainer",
);
...
export default definePlugin({
    ...
    renderName: ErrorBoundary.wrap(
        ({ session }: SessionInfo) => {
            ...
            const state = React.useState(
                savedSession?.name
                    ? `${savedSession.name}*`
                    : getDefaultName(session.client_info),
            );
    ...
    renderTimestamp: ErrorBoundary.wrap(
        ({ session, timeLabel }: { session: Session; timeLabel: string }) => {
            return (
                <Tooltip
            ...
            return (
                ...
                >
                    <div
                        ...
                        style={{
                            backgroundColor: GetOsColor(session.client_info.os),
                        }}
                    >
                        <DeviceIcon
                            width={28}
        ...
        USER_SETTINGS_ACCOUNT_RESET_AND_CLOSE_FORM() {
            ...
            if (lastFetchedHashes.length > 0) {
                savedSessionsCache.forEach((_, idHash) => {
                    if (!lastFetchedHashes.includes(idHash))
    ...
    async start() {
        ...
        if (settings.store.backgroundCheck) {
            this.checkInterval = setInterval(
                this.checkNewSessions,
                settings.store.checkInterval * 60 * 1000,
            );
        }
    },
