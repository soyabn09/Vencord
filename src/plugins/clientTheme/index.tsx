...
const saveClientTheme = findByCodeLazy(
...
function ThemeSettings() {
    ...
    const nitroTheme = useStateFromStores(
        [NitroThemeStore],
        () => NitroThemeStore.gradientPreset,
    );
    ...
    let contrastWarning = false,
        fixableContrast = true;
    if (
        (isLightTheme && selectedLuminance < 0.26) ||
        (!isLightTheme && selectedLuminance > 0.12)
    )
        ...
        <div className="client-theme-settings">
            ...
            {(contrastWarning || nitroThemeEnabled) && (
                <>
                    ...
                    />
                    <div
                        className={`client-theme-contrast-warning ${contrastWarning ? (isLightTheme ? "theme-dark" : "theme-light") : ""}`}
                    >
...
function genThemeSpecificOffsets(
    variableLightness: Record<string, number>,
    regex: RegExp,
    centerVariable: string,
): string {
    return Object.entries(variableLightness)
        .filter(([key]) => key.search(regex) > -1)
        .map(([key, lightness]) => {
            const lightnessOffset =
                lightness - variableLightness[centerVariable];
...
function generateLightModeFixes(styles) {
    const groupLightUsesW500Regex =
        /\.theme-light[^{]*\{[^}]*var\(--white-500\)[^}]*}/gm;
    ...
    const groupBackgroundColorRegex =
        /^([^{]*)\{background-color:var\(--white-500\)/m;
    // find all capturing groups that assign background or background-color directly to w500
    const backgroundGroups = mapReject(relevantStyles, (entry) =>
        captureOne(entry, groupBackgroundRegex),
    ).join(",\n");
    const backgroundColorGroups = mapReject(relevantStyles, (entry) =>
        captureOne(entry, groupBackgroundColorRegex),
    ).join(",\n");
    ...
    const lightVars = mapReject(relevantStyles, (style) =>
        captureOne(style, groupBgVarRegex),
    ) // get the insides of capture groups that have at least one background var with w500
        .map((str) => str.split(";"))
        .flat(); // captureGroupInsides[] -> cssRule[]
    const lightBgVars = mapReject(lightVars, (variable) =>
        captureOne(variable, bgVarRegex),
    ); // remove vars that aren't for backgrounds or w500
    ...
    createStyleSheet(
        ...
        [reassignBackgrounds, reassignBackgroundColors, reassignVariables].join(
            "\n\n",
        ),
    );
}
...
function updateColorVars(color: string) {
    ...
    if (!style) style = createStyleSheet("clientThemeVars");
