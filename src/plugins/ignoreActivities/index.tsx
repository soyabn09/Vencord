    ...
    return (
        <Flex flexDirection="column">
            <Forms.FormText type={Forms.FormText.Types.DESCRIPTION}>
                Import the application id of the CustomRPC plugin to the filter
                list
            </Forms.FormText>
            <div>
                <Button
                    onClick={() => {
                        const id = Settings.plugins.CustomRPC?.appID as
                            | string
                            | undefined;
                        if (!id) {
                            return showToast(
...
function IdsListComponent(props: { setValue: (value: string) => void }) {
    ...
    idsListPushID = (id: string) => {
        ...
        const isAlreadyAdded =
            currentIds.has(id) || (currentIds.add(id), false);
    ...
    return (
        <Forms.FormSection>
            ...
            >
                Comma separated list of activity IDs to filter (Useful for
                filtering specific RPC activities and CustomRPC
            </Forms.FormText>
            <TextInput
                type="text"
                value={idsList}
...
export default definePlugin({
    ...
    patches: [
            ...
            replacement: [
                {
                    ...
                    replace: (m, activities) =>
                        `${m}${activities}=${activities}.filter($self.isActivityNotIgnored);`,
                },
            ],
        },
        {
        ...
        // Discord has 2 different components for activities. Currently, the last is the one being used
        {
            ...
            replacement: {
                ...
                replace: (m, props) =>
                    `${m}$self.renderToggleActivityButton(${props}),`,
            },
        },
        {
    ...
    async start() {
        ...
        if (Settings.plugins.IgnoreActivities.allowedIds) {
            settings.store.idsList =
                Settings.plugins.IgnoreActivities.allowedIds;
            delete Settings.plugins.IgnoreActivities.allowedIds; // Remove allowedIds
        }

        const oldIgnoredActivitiesData = await DataStore.get<
            Map<IgnoredActivity["id"], IgnoredActivity>
        ...
        if (getIgnoredActivities().length !== 0) {
            const gamesSeen = RunningGameStore.getGamesSeen() as {
                id?: string;
                exePath: string;
            }[];
            ...
            for (const [
                index,
                ignoredActivity,
            ] of getIgnoredActivities().entries()) {
