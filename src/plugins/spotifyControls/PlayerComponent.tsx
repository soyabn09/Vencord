...
function makeContextMenu(name: string, path: string) {
    return (e: React.MouseEvent<HTMLElement, MouseEvent>) =>
        ContextMenuApi.openContextMenu(e, () => (
            <CopyContextMenu name={name} path={path} />
        ));
}
    ...
    return (
        <Flex className={cl("button-row")} style={{ gap: 0 }}>
            <Button
                className={classes(
                    cl("button"),
                    cl(shuffle ? "shuffle-on" : "shuffle-off"),
                )}
            ...
            >
                {repeat === "track" && (
                    <span className={cl("repeat-1")}>1</span>
                )}
                <Repeat />
            </Button>
        </Flex>
...
function SeekBar() {
    ...
    const [storePosition, isSettingPosition, isPlaying] = useStateFromStores(
        ...
        () => [
            ...
            SpotifyStore.isSettingPosition,
            SpotifyStore.isPlaying,
        ],
    );
...
export function Player() {
    const track = useStateFromStores(
        ...
        null,
        (prev, next) =>
            prev?.id ? prev.id === next?.id : prev?.name === next?.name,
    );
