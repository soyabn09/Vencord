...
async function generateDebugInfoMessage() {
    ...
    if (IS_DISCORD_DESKTOP) {
        info["Last Crash Reason"] =
            (
                await tryOrElse(
                    () => DiscordNative.processUtils.getLastCrash(),
                    undefined,
                )
            )?.rendererCrashReason ?? "N/A";
    }

    const commonIssues = {
        "NoRPC enabled": Vencord.Plugins.isPluginEnabled("NoRPC"),
        "Activity Sharing disabled": tryOrElse(
...
function generatePluginList() {
    ...
    const enabledPlugins = Object.keys(plugins).filter(
        (p) => Vencord.Plugins.isPluginEnabled(p) && !isApiPlugin(p),
    );

    const enabledStockPlugins = enabledPlugins.filter(
        (p) => !PluginMeta[p].userPlugin,
    );
    const enabledUserPlugins = enabledPlugins.filter(
        (p) => PluginMeta[p].userPlugin,
    );
...
export default definePlugin({
    ...
    commands: [
        {
            ...
            predicate: (ctx) =>
                ...
                AllowedChannelIds.includes(ctx.channel.id),
            execute: async () => ({
                content: await generateDebugInfoMessage(),
            }),
        },
        {
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
                ...
                if (isOutdated) {
                    return Alerts.show({
                        ...
                        body: (
                            <div>
                                ...
                                <Forms.FormText className={Margins.top8}>
                                    Please first update before asking for
                                    support!
                                </Forms.FormText>
                            </div>
                        ),
                        ...
                        secondaryConfirmText:
                            "I know what I'm doing or I can't update",
                    });
                }
            }
            ...
            const roles =
                GuildMemberStore.getSelfMember(VENCORD_GUILD_ID)?.roles;
            if (!roles || TrustedRolesIds.some((id) => roles.includes(id)))
                return;
                ...
                return Alerts.show({
                    ...
                    body: (
                        <div>
                            ...
                            >
                                You will be banned from receiving support if you
                                ignore this rule.
                            </Text>
                        </div>
                    ),
                    ...
                    onConfirmSecondary: () =>
                        (settings.store.dismissedDevBuildWarning = true),
                });
            }
        },
    },
            ...
            const shouldAddUpdateButton =
                ...
                (props.channel.id === KNOWN_ISSUES_CHANNEL_ID ||
                    (props.channel.id === SUPPORT_CHANNEL_ID &&
                        props.message.author.id === VENBOT_USER_ID)) &&
