...
async function bulkFetchPronouns(ids: string[]): Promise<PronounsResponse> {
    ...
    try {
        const req = await fetch(
            "https://pronoundb.org/api/v2/lookup?" + String(params),
            {
                method: "GET",
                headers: {
        ...
        const dummyPronouns: PronounsResponse = Object.fromEntries(
            ids.map((id) => [id, { sets: {} }]),
        );
...
function extractPronouns(pronounSets?: PronounSets): string | undefined {
    ...
    if (
        ...
        ["any", "ask", "avoid", "other", "unspecified"].includes(pronoun)
    ) {
