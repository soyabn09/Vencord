...
const OptionClasses = findByPropsLazy(
    "optionName",
    "optionIcon",
    "optionLabel",
);
...
const VoiceRecorder = IS_DISCORD_DESKTOP
...
const ctxMenuPatch: NavContextMenuPatchCallback = (children, props) => {
    if (
        ...
        !(
            PermissionStore.can(
                PermissionsBits.SEND_VOICE_MESSAGES,
                props.channel,
            ) &&
...
function sendAudio(blob: Blob, meta: AudioMetadata) {
    ...
    const upload = new CloudUpload(
        {
            file: new File([blob], "voice-message.ogg", {
    ...
    upload.on("error", () =>
        showToast("Failed to upload voice message", Toasts.Type.FAILURE),
    );
...
function Modal({ modalProps }: { modalProps: ModalProps }) {
    ...
    const [meta] = useAwaiter(
        async () => {
            ...
            const ratio =
                1 +
                (0xff / maxBin - 1) * Math.min(1, 100 * (maxBin / 0xff) ** 3);
            for (let i = 0; i < bins.length; i++)
                bins[i] = Math.min(0xff, ~~(bins[i] * ratio));
    ...
    return (
        <ModalRoot {...modalProps}>
            ...
            <ModalContent className={cl("modal")}>
                ...
                {isUnsupportedFormat && (
                    <Card
                        className={`vc-plugins-restart-card ${Margins.top16}`}
                    >
                        <Forms.FormText>
                            Voice Messages have to be OggOpus to be playable on
                            iOS. This file is <code>{blob.type}</code> so it
                            will not be playable on iOS.
                        </Forms.FormText>

                        <Forms.FormText className={Margins.top8}>
                            To fix it, first convert it to OggOpus, for example
                            using the{" "}
                            <Link href="https://convertio.co/mp3-opus/">
