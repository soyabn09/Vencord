...
function calculateIdx(messages: Message[], id: string) {
    ...
    return idx === -1 ? idx : messages.length - idx - 1;
}
...
function getNextMessage(isUp: boolean, isReply: boolean) {
    let messages: Array<Message & { deleted?: boolean }> =
    ...
    const mutate = (i: number) =>
        isUp ? Math.min(messages.length - 1, i + 1) : Math.max(-1, i - 1);
    ...
    const findNextNonDeleted = (i: number) => {
        do {
            ...
            i !== -1 &&
            messages[messages.length - i - 1]?.deleted === true
        );
        return i;
    };

    let i: number;
    if (isReply) replyIdx = i = findNextNonDeleted(replyIdx);
    else editIdx = i = findNextNonDeleted(editIdx);

    return i === -1 ? undefined : messages[messages.length - i - 1];
}

function shouldMention(message) {
    const { enabled, userList, shouldPingListed } =
        Settings.plugins.NoReplyMention;
    const shouldPing =
        !enabled || shouldPingListed === userList.includes(message.author.id);
...
function nextReply(isUp: boolean) {
    ...
    if (
        ...
        !PermissionStore.can(PermissionsBits.SEND_MESSAGES, currChannel)
    )
        return;
