        ...
        options: [
            {
                ...
                value: PinOrder.LastMessage,
                default: true,
            },
            {
                label: "Custom (right click channels to reorder)",
                value: PinOrder.Custom,
            },
        ],
...
export default definePlugin({
    ...
    getChunkSize() {
        ...
        return (
            (sectionHeaderSizePx +
                sections.reduce((acc, v) => (acc += v + 44), 0) +
                DEFAULT_CHUNK_SIZE) *
            1.5
        );
    },
    ...
    isChannelIndex(sectionIndex: number, channelIndex: number) {
        ...
        return (
            this.isCategoryIndex(sectionIndex) &&
            (cat?.channels?.length === 0 || cat?.channels[channelIndex])
        );
    },
    ...
    isChannelHidden(categoryIndex: number, channelIndex: number) {
        ...
        if (!this.instance || !this.isChannelIndex(categoryIndex, channelIndex))
            return false;
            ...
            () => {
                ...
                return (
                    <ChannelComponent
                        channel={channel}
                        selected={
                            this.instance.props.selectedChannelId === channel.id
                        }
                    >
                        {channel.id}
                    </ChannelComponent>
