...
export async function initCategories(userId: string) {
    categories =
        (await DataStore.get<Category[]>(CATEGORY_BASE_KEY + userId)) ?? [];
}
...
// migrate data
const getPinDMsPins = () =>
    (Settings.plugins.PinDMs.pinnedDMs || void 0)?.split(",") as
        | string[]
        | undefined;
...
async function migratePinDMs() {
    ...
    const difference = [...new Set(pindmspins)]?.filter(
        (m) => !categories.some((c) => c.channels.includes(m)),
    );
...
export async function migrateData(userId: string) {
    const m1 = await DataStore.get(CATEGORY_MIGRATED_KEY),
