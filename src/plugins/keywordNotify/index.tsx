...
const settings = definePluginSettings({
    ...
    whiteList: {
        ...
        description: "Comma-separated list of IDs to whitelist",
        default: ""
    },

    keywords: {
        type: OptionType.COMPONENT,
        description: "Keyword regexes to receive notifications and highlight messages",
        component: () => {
...
export default definePlugin({
    ...
    applyRegexes(m) {
        const { ignoreBots, ignoreSelf, whiteList } = settings.store;
        const currentUserID = UserStore.getCurrentUser().id;

        // Message attributes
        const authorID = m.author?.id;
        const channelID = m.channel_id;
        const channelInfo = ChannelStore.getChannel(channelID);
        const parentID = channelInfo?.parent_id;
        const guildID = channelInfo?.guild_id;

        // Ignoring conditions
        const isBot = ignoreBots && m.author?.bot;
        const isSelf = ignoreSelf && authorID === currentUserID;
        const WhiteList = whiteList.includes(authorID) || whiteList.includes(channelID) || whiteList.includes(parentID) || whiteList.includes(guildID);
