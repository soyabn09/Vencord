...
function parseDevs() {
    ...
    for (const child of file.getChildAt(0).getChildren()) {
        ...
        const devsDeclaration = child.declarationList.declarations.find((d) =>
            hasName(d, "Devs"),
        );
        if (
            !devsDeclaration?.initializer ||
            !isCallExpression(devsDeclaration.initializer)
        )
            continue;
        ...
        if (
            !isSatisfiesExpression(value) ||
            !isObjectLiteralExpression(value.expression)
        )
        ...
        for (const prop of value.expression.properties) {
            ...
            if (!isObjectLiteralExpression(value))
                throw new Error(
                    `Failed to parse devs: ${name} is not an object literal`,
                );

            devs[name] = {
                name: (getObjectProp(value, "name") as StringLiteral).text,
                id: (getObjectProp(value, "id") as BigIntLiteral).text.slice(
                    0,
                    -1,
                ),
            };
...
async function parseFile(fileName: string) {
    ...
    for (const node of file.getChildAt(0).getChildren()) {
        ...
        if (
            ...
            call.expression.text !== "definePlugin"
        )
            continue;
            ...
            switch (key) {
                ...
                case "authors":
                    ...
                    data.authors = value.elements.map((e) => {
                        if (!isPropertyAccessExpression(e))
                            throw fail(
                                "authors array contains non-property access expressions",
                            );
                ...
                case "tags":
                    ...
                    data.tags = value.elements.map((e) => {
                        if (!isStringLiteral(e))
                            throw fail(
                                "tags array contains non-string literals",
                            );
                        return e.text;
                    });
                    break;
                case "dependencies":
                    ...
                    if (elements.some((e) => !isStringLiteral(e)))
                        throw fail(
                            "dependencies array contains non-string elements",
                        );
                    data.dependencies = (
                        elements as NodeArray<StringLiteral>
                    ).map((e) => e.text);
                    break;
                case "required":
...
(async () => {
    ...
    await Promise.all(
        ["src/plugins", "src/plugins/_core"].flatMap((dir) =>
            readdirSync(dir, { withFileTypes: true })
                .filter(isPluginFile)
                .map(async (dirent) => {
                    const [data, readme] = await parseFile(
                        await getEntryPoint(dir, dirent),
                    );
                    plugins.push(data);
